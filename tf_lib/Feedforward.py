#  ***********************************************************************
#   Description: This file defines some common feedforward Neural Network
#   arquitectures:
...
class ConvNet(object):
    ...
    def setup(self, batch_size, drop_prob=None, l2_reg_coef=None, loss_type=None):
        ...
        if loss_type is None:
            ...
            if self.n_outputs == 1:
                loss = tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(y, labels)) + l2_reg
            else:
                loss = tf.reduce_mean(
                    tf.nn.softmax_cross_entropy_with_logits(y, labels)) + l2_reg
        elif loss_type == 'l2':
...
class MlpNet(object):
    ...
    def setup(self, batch_size=None, drop_prob=None, l2_reg_coef=None,
        ...
        @param drop_prob: placeholder used for specify the probability for
                          ...
                          to the list.
        @param l2_reg_coef: coeficient for l2 regularization
        @param loss_type: type of the loss being used for training the network,
                          the options are:
                          -'cross_entropy': for classification tasks
                          -'l2': for regression tasks
        '''
        ...
        # Get list of dropout between activation layers
        if (not isinstance(drop_prob, list)):
            # by default, dropout is not applied in the input layer
            drop_prob = [None] + \
                        [drop_prob for i in range(len(self.full_layers))]
        # input layer
        if drop_prob[0] is None:
            out = inputs
        else:
            out = tf.nn.dropout(inputs, drop_prob[0])
        for (layer, p_i) in zip(self.full_layers, drop_prob[1:]):
            out = layer.evaluate(out)
            if p_i is not None:
                out = tf.nn.dropout(out, p_i)
        ...
        if loss_type is None:
            ...
            loss = 0.5 * tf.reduce_mean(
                tf.reduce_sum(tf.pow(y - labels, 2), 1), 0) + l2_reg
...
class StridedConvNet(object):   # TODO!!!!!!!!!!!!!!!!!!!
    ...
    def __init__(self, input_size, n_input_maps, n_outputs,
        ...
        self.conv_layers = list()
        self.conv_layers.append(
            StridedConvLayer(filter_size[0], [
                             n_input_maps, n_filters[0]], strides[0], name='0_conv_' + name)
        )

        for l in range(1, len(n_filters)):
            self.conv_layers.append(
                StridedConvLayer(filter_size[l], [
                                 n_filters[l - 1], n_filters[l]], strides[l],
                                 name=str(l) + '_conv_' + name)
            )
        ...
        if len(n_hidden) > 0:
            self.full_layers.append(
                FullyconnectedLayer(
                    final_size[0] * final_size[1] * n_filters[-1], n_hidden[0],
                    name='0_full_' + name)
            )
            for l in range(1, len(n_hidden)):
                self.full_layers.append(
                    FullyconnectedLayer(
                        n_hidden[l - 1], n_hidden[l],
                        name=str(l) + '_full_' + name)
                )
            ...
            # 3. Create the final layer: (TODO: test this!!!!!!)
            self.out_layer = AffineLayer(
                final_size[0] * final_size[1] * n_filters[-1], n_outputs,
                name='_lin_' + name)
